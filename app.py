...
st.set_page_config(page_title="Распознавание языка жестов", layout="wide")
st.title("Распознавание языка жестов")
    ...
    if frame is not None:
        ...
        if label_counters and (time.time() - start_time) > 1:
            ...
            if label_counters[max_label] >= 5 and max_label != last_file_label:
                ...
                recognized_text.markdown(f"<h3>Распознанные буквы: {''.join(cleaned_letters)}</h3>", unsafe_allow_html=True)
            label_counters = {}
            prev_label = None
            start_time = time.time()
